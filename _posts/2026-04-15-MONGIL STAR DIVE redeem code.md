---
layout: post
title: "MONGIL: STAR DIVE – Redeem Codes"
subtitle: "The Next Generation of Monster-Taming Action RPGs"
date: 2026-04-15T00:00:00Z
release_date: "2026-04-15"
category: codes
badge: redeem code
tags:
 - Action RPG
 - Gacha
 - Netmarble
image: https://i.ibb.co/23DfJ6H7/mongil-star-dive-ios-android-cbt-cover.jpg
description: "MONGIL: STAR DIVE, the highly anticipated action RPG developed by Netmarble Monster Inc., is officially right around the corner. Set for a global launch on April 15, 2026 (with US servers opening on April 14), this vibrant title promises to revive Netmarble's classic monster-taming legacy. Blending fast-paced hack-and-slash combat, deep gacha mechanics, and a charming creature-collection system, MONGIL: STAR DIVE is gearing up to be one of the biggest multi-platform releases of the spring season."

hero_image: https://i.ibb.co/23DfJ6H7/mongil-star-dive-ios-android-cbt-cover.jpg

info:
  type: Gift Codes
  status: Active
  update_frequency: Weekly
  developer: Netmarble

images:
  - https://i.ibb.co/whXz5N3j/unnamed-12.jpg
  - https://i.ibb.co/xSRJMH2f/unnamed-13.jpg
  - https://i.ibb.co/rfvcBG6w/unnamed-14.jpg

video_id: IMAIWO9EaE0

buttons:
  - label: "Play Store"
    url: "https://play.google.com/store/apps/details?id=com.netmarble.monster2"
    type: playstore

---
## 🌟 Start Your Journey with MONGIL: STAR DIVE Codes

To celebrate the global launch of MONGIL: STAR DIVE, Netmarble has released several promotional codes to help new Tamers strengthen their team. Whether you need Diamonds for summons, Gold, or even free Monsterlings, you'll find everything you need here.

---

### ✅ Active Codes (April 2026)
Here are the currently working codes. Make sure to type them exactly as they appear (the system is case-sensitive).

| Code                        | Rewards |
| :---------------------------| :--- |
| **MONGILDISCORD**           | 30 Diamonds, 30 Glimmering Stardust, 300,000 Gold |
| **LOVEMONGIL**              | Overflowing Love (4-Star Supporter Artifact) |
| **DEVLIVE0410**             | Scar, White Wolf Warrior, and Spark Slime (Monsterlings) |
| **MONGILPRESENT**           | Brown Cappy, Spark Slime, and Forkmugger (Monsterlings) |
| **DINDINISDINDIN**          | Green Cappy (Monsterling) and 10,000 Gold |
| **MONGILREPORT**            | White Wolf Warrior (Monsterling) |

---

### ❌ Expired Codes
The following codes are no longer valid. Don't waste your time trying them!
*No expired codes yet. Check back soon!*

---

🛠 How to Redeem Codes in MONGIL: STAR DIVE
Claiming your rewards is easy. Just follow these steps within the game:

Launch the game and complete the initial "Monster Tamers" tutorial.

Open the Menu (located in the top-right corner of the screen).

Select the Settings button (the gear icon).

Navigate to the Other tab on the sidebar.

Scroll down to the Customer Support section and tap Redemption Code.

Enter one of the codes listed above into the text field.

Confirm and check your In-game Mailbox to collect your gifts!

Note: Each code can only be used once per account. If a code doesn't work, ensure you haven't added any extra spaces at the end of the text. iOS users may need to redeem codes via the official Netmarble redemption website using their Player ID.