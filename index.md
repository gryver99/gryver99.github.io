  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .navbar {
      ...
      box-shadow: 0 2px 24px rgba(0,0,0,.5), 0 1px 0 rgba(57,255,20,0.07);
    }
    .nav-brand {
      display: flex; align-items: center; gap: .55rem;
      font-family: 'Cinzel', serif; font-size: 1.1rem;
      color: var(--bright); letter-spacing: .06em; transition: color .2s;
    }
    ...
    .nav-links a.active { text-shadow: 0 0 10px var(--neon-glow); }

    .dropdown { position: relative; }
    .dropdown-menu {
      display: none; position: absolute; top: 40px; left: 50%; transform: translateX(-50%);
      background: var(--surface2); border: 1px solid var(--border); border-radius: var(--r);
      min-width: 150px; overflow: hidden; box-shadow: 0 8px 24px rgba(0,0,0,.7);
    }
    .dropdown:hover .dropdown-menu { display: block; }
    .dropdown-menu a { display: block; padding: .6rem 1.1rem; font-size: .78rem; color: var(--text); }
    .dropdown-menu a:hover { background: var(--surface); color: var(--neon); }

    .nav-yt {
      display: flex; align-items: center; gap: .4rem;
      background: rgba(255,0,0,0.1); border: 1px solid rgba(255,0,0,0.3);
      border-radius: var(--r); padding: .28rem .75rem;
      color: #ff4444 !important; font-size: .75rem !important;
      transition: all .2s !important;
    }
    .nav-yt:hover { background: rgba(255,0,0,0.22) !important; border-color: rgba(255,0,0,0.6) !important; box-shadow: 0 0 12px rgba(255,0,0,0.2) !important; }
    .nav-yt svg { width: 14px; height: 14px; fill: currentColor; }

    .nav-search {
      background: none; border: 1px solid transparent; cursor: pointer;
      color: var(--muted); padding: .3rem; border-radius: var(--r);
      transition: all .2s; display: flex; align-items: center;
    }
    .nav-search:hover { color: var(--neon); border-color: var(--border); }
    .nav-search svg { width: 18px; height: 18px; stroke: currentColor; fill: none; stroke-width: 2; }
    ...
    .hero-bg {
      ...
      background: radial-gradient(ellipse 70% 60% at 50% 50%, rgba(57,255,20,0.07) 0%, transparent 70%),
    ...
    .hero-title {
      ...
      text-shadow: 0 0 40px rgba(57,255,20,0.22);
    }
    .hero-title span { color: var(--neon); }
    .hero-sub {
      margin-top: .5rem; font-family: 'Space Mono', monospace;
    ...
    /* ══ CAROUSEL ══ */
    .carousel-section { padding: 2rem 0; border-bottom: 1px solid var(--border); }

    .carousel-outer {
      position: relative; border: 1px solid var(--border); border-radius: var(--r);
      overflow: hidden; background: var(--surface);
      box-shadow: 0 0 0 1px rgba(57,255,20,0.05), 0 8px 32px rgba(0,0,0,.5);
    }
    ...
    .c-slide img { width:100%; height:170px; object-fit:cover; transition:transform .5s,opacity .3s; opacity:0; }
    .c-slide img.loaded { opacity:1; }
    .c-slide:hover img { transform:scale(1.06); }
    .c-overlay { position:absolute;inset:0; background:linear-gradient(to top,rgba(0,0,0,.88),rgba(0,0,0,.25) 55%,transparent); }
    .c-label {
      position:absolute; bottom:0;left:0;right:0;
      padding:.6rem .75rem; font-size:.75rem; font-weight:700;
      color:#fff; line-height:1.3; z-index:2; text-shadow:0 1px 4px rgba(0,0,0,.9);
    }
    .c-btn {
      position:absolute; top:50%; transform:translateY(-50%);
      background:rgba(0,0,0,.75); border:1px solid rgba(57,255,20,0.4); color:var(--neon);
      width:32px;height:32px; cursor:pointer; z-index:10;
      display:flex;align-items:center;justify-content:center; font-size:.9rem;
      clip-path:polygon(0 0,calc(100% - 6px) 0,100% 6px,100% 100%,6px 100%,0 calc(100% - 6px));
      transition:all .2s;
    }
    .c-btn:hover { background:var(--neon); color:#000; box-shadow:0 0 14px var(--neon-glow); }
    .c-btn.prev { left:8px; } .c-btn.next { right:8px; }
    .c-dots { display:flex; justify-content:center; gap:6px; margin-top:.75rem; }
    .c-dot {
      width:8px;height:8px;border-radius:50%; border:none; cursor:pointer;
      background:rgba(57,255,20,0.2); border:1px solid rgba(57,255,20,0.3); transition:all .25s;
    }
    .c-dot.active { background:var(--neon); box-shadow:0 0 8px var(--neon-glow); transform:scale(1.3); }

    /* ══ YOUTUBE WIDGET ══ */
    .yt-section { padding: 2.5rem 0; border-bottom: 1px solid var(--border); }
    .gl-yt-widget { position:relative; width:100%; padding:0 42px; }
    .gl-video-grid { display:grid; grid-template-columns:repeat(3,1fr); gap:10px; }
    ...
    .gl-video-card:hover { transform:translateY(-5px); border-color:var(--neon); box-shadow:0 0 0 1px var(--neon),0 12px 28px rgba(57,255,20,0.2); }
    ...
    .gl-video-card:hover .gl-ov { background:rgba(0,0,0,.12); }
    .gl-play {
      position:absolute; top:38%;left:50%; transform:translate(-50%,-50%);
      width:38px;height:38px; background:rgba(0,0,0,.35); border-radius:50%;
      border:1.5px solid rgba(57,255,20,0.5); display:flex;align-items:center;justify-content:center;
      z-index:4; transition:all .3s; box-shadow:0 0 8px rgba(57,255,20,0.2); backdrop-filter:blur(2px);
    }
    .gl-video-card:hover .gl-play { transform:translate(-50%,-50%) scale(1.15); background:rgba(57,255,20,0.85); border-color:var(--neon); box-shadow:0 0 20px var(--neon-glow); }
    .gl-play::before { content:''; width:0;height:0; border-style:solid; border-width:6px 0 6px 11px; border-color:transparent transparent transparent rgba(57,255,20,0.8); margin-left:3px; transition:border-color .3s; }
    .gl-video-card:hover .gl-play::before { border-color:transparent transparent transparent #000; }
    .gl-vbody { position:absolute;bottom:0;left:0;right:0;padding:8px 8px 10px;z-index:4; }
    .gl-vtitle { margin:0; font-family:'Rajdhani',sans-serif; font-size:.88rem;font-weight:700; color:#fff;line-height:1.2;letter-spacing:.02em; text-shadow:0 1px 6px rgba(0,0,0,.8); display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden; }

    .gl-nav-btn { position:absolute; top:50%;transform:translateY(-50%); background:rgba(0,0,0,.85); border:1px solid rgba(57,255,20,0.5); color:var(--neon); width:34px;height:34px; cursor:pointer;z-index:10; display:flex;align-items:center;justify-content:center; font-size:13px; transition:all .25s; clip-path:polygon(0 0,calc(100% - 7px) 0,100% 7px,100% 100%,7px 100%,0 calc(100% - 7px)); }
    .gl-nav-btn:hover { background:var(--neon);color:#000;box-shadow:0 0 14px var(--neon-glow); }
    ...
    .gl-dot { width:6px;height:6px;border-radius:50%;background:rgba(57,255,20,.2);border:1px solid rgba(57,255,20,.3);transition:all .3s;cursor:pointer; }
    ...
    @keyframes glR { from{transform:translateX(25px);opacity:0} to{transform:translateX(0);opacity:1} }
    @keyframes glL { from{transform:translateX(-25px);opacity:0} to{transform:translateX(0);opacity:1} }

    /* ══ NEWS GRID ══ */
    .news-section { padding: 2.5rem 0 3rem; }

    .gl-search-wrap { position:relative; margin-bottom:.9rem; }
    .gl-search-icon { position:absolute;left:14px;top:50%;transform:translateY(-50%);color:var(--neon);font-size:.9rem;pointer-events:none; }
    .gl-search-input {
      width:100%; padding:10px 18px 10px 38px;
      background:rgba(0,0,0,.7); border:1px solid rgba(57,255,20,.4); border-radius:var(--r);
      color:#fff; font-size:.85rem; font-family:'Space Mono',monospace; letter-spacing:.04em;
      outline:none; transition:all .3s;
      clip-path:polygon(0 0,calc(100% - 10px) 0,100% 10px,100% 100%,10px 100%,0 calc(100% - 10px));
    }
    .gl-search-input::placeholder { color:rgba(57,255,20,.3); }
    .gl-search-input:focus { border-color:var(--neon); box-shadow:0 0 18px var(--neon-glow),inset 0 0 8px rgba(57,255,20,.04); }

    .gl-filter-bar { display:flex;align-items:center;gap:5px;flex-wrap:nowrap;overflow-x:auto;scrollbar-width:none;margin-bottom:1rem;padding-bottom:4px; }
    .gl-filter-bar::-webkit-scrollbar { display:none; }
    .gl-filter-label { font-family:'Space Mono',monospace;font-size:.55rem;color:var(--muted);letter-spacing:1.5px;text-transform:uppercase;white-space:nowrap;margin-right:4px; }
    .gl-filter-btn {
      font-family:'Rajdhani',sans-serif;font-size:.63rem;font-weight:700;letter-spacing:1px;text-transform:uppercase;
      padding:4px 11px;border-radius:2px;cursor:pointer;white-space:nowrap;flex-shrink:0;
      background:rgba(0,0,0,.5);color:var(--muted);border:1px solid rgba(255,255,255,.1);transition:all .2s;
      clip-path:polygon(0 0,calc(100% - 5px) 0,100% 5px,100% 100%,5px 100%,0 calc(100% - 5px));
    }
    .gl-filter-btn:hover { color:rgba(255,255,255,.7);border-color:rgba(255,255,255,.25); }
    .gl-filter-btn.all.active   { background:var(--neon-dim);color:var(--neon);border-color:rgba(57,255,20,.5);box-shadow:0 0 8px var(--neon-glow); }
    ...
    .gl-news-grid { display:grid;grid-template-columns:repeat(3,1fr);gap:10px; }
    .gl-link-w { text-decoration:none;color:inherit;display:block;height:280px;opacity:0;animation:fadeIn .5s ease forwards; }
    ...
    .gl-link-w:hover .gl-card { transform:translateY(-5px);border-color:var(--neon);box-shadow:0 0 0 1px var(--neon),0 12px 30px rgba(57,255,20,.18); }

    .gl-thumb { position:relative;width:100%;height:100%;background:var(--surface);overflow:hidden;flex-shrink:0; }
    .gl-thumb::before { content:'';position:absolute;inset:0;background:linear-gradient(90deg,#111 25%,#1a1a1a 50%,#111 75%);background-size:200% 100%;animation:shimmer 1.5s infinite; }
    ...
    .gl-thumb::after { content:'';position:absolute;bottom:0;left:0;width:100%;height:70%;background:linear-gradient(to top,rgba(0,0,0,.92),rgba(0,0,0,.5) 50%,transparent);z-index:2; }

    .gl-category { position:absolute;top:7px;left:7px;padding:2px 7px;font-size:.56rem;font-weight:700;font-family:'Rajdhani',sans-serif;text-transform:uppercase;letter-spacing:1.2px;border-radius:2px;z-index:3;backdrop-filter:blur(4px);background:rgba(0,221,180,.12);color:var(--teal);border:1px solid rgba(0,221,180,.45); }
    ...
    .gl-body { position:absolute;bottom:0;left:0;right:0;padding:8px 8px 26px 8px;z-index:3; }
    .gl-date { color:var(--neon);font-size:.58rem;font-weight:700;font-family:'Space Mono',monospace;display:block;margin-bottom:3px;letter-spacing:.04em;opacity:.85; }
    .gl-title { margin:0;font-family:'Rajdhani',sans-serif;font-size:.98rem;font-weight:700;line-height:1.15;color:#fff;text-shadow:0 1px 6px rgba(0,0,0,.8);display:-webkit-box;-webkit-line-clamp:3;-webkit-box-orient:vertical;overflow:hidden; }

    .gl-badge { position:absolute;bottom:6px;right:6px;padding:2px 7px;font-size:.52rem;font-weight:700;font-family:'Rajdhani',sans-serif;text-transform:uppercase;letter-spacing:1.2px;z-index:4;border-radius:2px;clip-path:polygon(0 0,calc(100% - 5px) 0,100% 5px,100% 100%,5px 100%,0 calc(100% - 5px)); }
    ...
    @keyframes betaP { 0%{box-shadow:0 0 6px rgba(255,140,50,.3)} 100%{box-shadow:0 0 14px rgba(255,140,50,.6)} }

    .gl-no-results { grid-column:1/-1;text-align:center;color:rgba(57,255,20,.35);font-family:'Space Mono',monospace;padding:60px;font-size:.8rem;letter-spacing:3px; }

    .gl-pagination { display:flex;justify-content:center;align-items:center;gap:5px;margin-top:1.4rem;flex-wrap:wrap; }
    .gl-page-btn { background:rgba(0,0,0,.6);color:var(--muted);border:1px solid rgba(255,255,255,.1);padding:5px 11px;min-width:34px;font-size:.78rem;font-weight:700;font-family:'Rajdhani',sans-serif;letter-spacing:1px;cursor:pointer;border-radius:3px;transition:all .2s; }
    ...
    .gl-page-btn.active { background:var(--gold-dim);color:var(--gold);border-color:var(--gold);box-shadow:0 0 12px rgba(200,168,75,.3); }
    .gl-page-btn.arrow { font-size:1rem;padding:4px 9px; }
    ...
    footer.gl-footer::before { content:'';position:absolute;inset:0;background:repeating-linear-gradient(0deg,transparent,transparent 3px,rgba(0,0,0,.07) 3px,rgba(0,0,0,.07) 4px);pointer-events:none; }
    .gl-ft-inner { position:relative;z-index:1;max-width:var(--max-w);margin:0 auto;padding:28px 48px;display:flex;align-items:center;justify-content:space-between;gap:24px; }

    .gl-ft-brand { display:flex;flex-direction:column;align-items:flex-start;gap:8px;flex:1; }
    .gl-ft-brand img { height:64px;width:auto;object-fit:contain; }
    .gl-ft-copy { font-size:11px;font-weight:600;letter-spacing:1.5px;text-transform:uppercase;color:var(--muted);line-height:1.6; }

    .gl-ft-nav { display:flex;flex-direction:column;align-items:center;gap:8px;flex:1; }
    .gl-ft-nav a { font-family:'Rajdhani',sans-serif;font-size:13px;font-weight:700;letter-spacing:1.8px;text-transform:uppercase;color:var(--text);padding:3px 10px;border:1px solid transparent;border-radius:3px;transition:color .2s,border-color .2s,text-shadow .2s; }
    .gl-ft-nav a:hover { color:var(--teal);border-color:rgba(0,221,180,.25);text-shadow:0 0 10px var(--teal-dim); }

    .gl-ft-right { display:flex;flex-direction:column;align-items:flex-end;gap:10px;flex:1; }
    .gl-status { display:flex;align-items:center;gap:6px;background:rgba(0,221,180,.07);border:1px solid rgba(0,221,180,.25);border-radius:20px;padding:4px 12px;font-size:9px;font-weight:700;letter-spacing:1.5px;text-transform:uppercase;color:var(--teal); }
    .gl-status-dot { width:6px;height:6px;border-radius:50%;background:var(--teal);animation:glpulse 2s ease-in-out infinite; }
    @keyframes glpulse { 0%,100%{opacity:1} 50%{opacity:.3} }
    .gl-social { display:flex;gap:10px; }
    .gl-social a { display:flex;align-items:center;justify-content:center;width:32px;height:32px;border:1px solid var(--border);border-radius:var(--r);background:rgba(255,255,255,.04);color:var(--text);transition:color .2s,border-color .2s,box-shadow .2s; }
    .gl-social a:hover { color:var(--teal);border-color:var(--teal-dim);box-shadow:0 0 18px var(--teal-glow); }
    .gl-social svg { width:15px;height:15px;fill:currentColor; }
    .gl-legal { display:flex;gap:14px;font-size:9px;font-weight:600;letter-spacing:1px;text-transform:uppercase;color:var(--muted); }
    .gl-legal a { color:inherit;transition:color .2s; }
    .gl-legal a:hover { color:var(--text); }

    /* ══ RESPONSIVE ══ */
    @media(max-width:900px){
      .gl-video-grid,.gl-news-grid { grid-template-columns:repeat(2,1fr); }
      .gl-link-w { height:220px; }
      .gl-title { font-size:.85rem;-webkit-line-clamp:2; }
    }
    @media(max-width:600px){
      .navbar { padding:0 1rem; }
      .nav-links li:nth-child(2){ display:none; }
      .gl-video-grid,.gl-news-grid { grid-template-columns:1fr; }
      .gl-link-w { height:170px; }
      .gl-yt-widget { padding:0 30px; }
      .c-slide { flex:0 0 100%; }
      .gl-ft-inner { padding:16px 20px; }
      .gl-ft-nav,.gl-status { display:none; }
      .gl-ft-brand img { height:36px; }
      .gl-ft-right { flex-direction:row;align-items:center;gap:10px; }
      .gl-legal { font-size:8px;gap:8px; }
    }
  </style>
</head>
<body>
...
<footer class="gl-footer" role="contentinfo">
  <div class="gl-ft-inner">
    <div class="gl-ft-brand">
      <img src="https://i.ibb.co/WNVG4yC6/Copilot-20260309-190533.png" alt="GameLeaf Logo" width="64" height="64" loading="lazy" onerror="this.style.display='none'">
...
(function(){
  ...
  const perV   = window.innerWidth < 601 ? 1 : 3;
  let pos = 0;
  ...
  function move(){
    track.style.transform = `translateX(-${pos*(100/perV)}%)`;
...
(function(){
  ...
  function renderPage(p){
    ...
    if(!filtered.length){
      ...
      document.getElementById('pagination-controls').innerHTML='';return;
    }
    const n=ipp();
    ...
    buildPag();
    if(p>1)window.scrollTo({top:document.querySelector('.news-section').offsetTop-70,behavior:'smooth'});
  }
